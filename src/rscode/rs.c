...
void
initialize_ecc (int parity_bytes)
{
  /* Initialize the galois field arithmetic tables */
    /* This is not using any kind of extern constant !! */
    init_galois_tables();

    /* Compute the encoder generator polynomial */
    /* This uses NPAR */
    /* genPoly is an array -> malloc that */
    NPAR = parity_bytes;
    MAXDEG = parity_bytes*2;
    genPoly = malloc(MAXDEG * sizeof(int));
    pBytes = malloc(NPAR * sizeof(int));
    synBytes = malloc(NPAR * sizeof(int));
    compute_genpoly(NPAR, genPoly);
    init_Berlekamp(MAXDEG);
}
